...
**Omni Translator Client** (Project ID: `Omni`) is a high-performance, modular translation plugin designed for Minecraft
networks. It enables seamless real-time translation of in-game content such as chat messages, signs, inventories, and
holograms, ensuring a localized experience for every player.

The project is built as a multi-module Maven project, featuring native integrations for **Spigot/Paper** servers and *
...
2. **`proxy`**:
   The Velocity-specific plugin (`VelocityBridge`). It handles cross-server connections, RESTful services, and
   network-wide MotD (Message of the Day) management with GeoIP support.
3. **`spigot`**:
   The backend plugin (`SpigotTranslator`) for individual game servers. It handles low-level packet manipulation for
   inventories, holograms, signs, and chat.

...
public class ProxyTransport {
    ...
    public ProxyTransport(VelocityTranslator velocityTranslator, String secret) {
        ...
        this.transportService = TransportService.<RegisteredServer>builder()
                ...
                .networkSender((channel, bytes) -> channel.sendPluginMessage(identifier, bytes))
                .build();

        velocityTranslator.getProxyServer().getEventManager().register(velocityTranslator, (PluginMessageListener) event -> {
    ...
    interface PluginMessageListener {
        ...
        @Subscribe
        void accept(PluginMessageEvent event);
    }
}

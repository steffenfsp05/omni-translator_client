...
public class DefaultPlaceholderNormalizer implements PlaceholderNormalizer {


    private static final Pattern CODE_PATTERN = Pattern.compile("(?:\\{C\\d+\\})+");
    private static final Pattern pattern = Pattern.compile("\\{C\\d+\\}");

    Cache<UUID, org.pytenix.placeholder.PlaceholderNormalizer.NormalizationResult> cachedNormalized = CacheBuilder.newBuilder()
            .expireAfterWrite(30, TimeUnit.SECONDS).build();


    public String normalizeText(UUID uuid, String text) {

        org.pytenix.placeholder.PlaceholderNormalizer.NormalizationResult normalizationResult = normalize(text);
    ...
    private org.pytenix.placeholder.PlaceholderNormalizer.NormalizationResult normalize(String text) {
        if (text == null || !text.contains("{C")) {
            return new org.pytenix.placeholder.PlaceholderNormalizer.NormalizationResult(text, Collections.emptyMap());
        }
        ...
        return new org.pytenix.placeholder.PlaceholderNormalizer.NormalizationResult(sb.toString(), translationMap);
    }
